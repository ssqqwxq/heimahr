      ...
      <el-form ref="dataForm" :model="formData" label-position="right" label-width="100px">
        <el-form-item label="应用：" prop="processType">
          <el-select v-model="formData.processType" class="filter-item" filterable>
            <el-option v-for="item in baseData.approvalType" :key="item.id" :label="item.value" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="节点：">
          <p><strong /><el-button size="small" type="primary" icon="el-icon-circle-plus-outline"
              @click="addTemp">新增节点</el-button></p>
          <div v-for="(item, index) in tempList" :key="item.key" :label="'域名' + index" :prop="'item.' + index + '.name'"
            ...
            <el-form-item label="执行人：" prop="formOfEmployment">
              <el-select v-model="item.user" class="filter-item" multiple style="width:300px;">
                <el-option v-for="item in getEmploySimpleData" :key="item.id" :label="item.fullName" :value="item.id" />
              </el-select>
            </el-form-item>
          </div>
        </el-form-item>
...
<script>
import { getManagerListApi } from '@/api/department'
...
export default {
  ...
  methods: {
    ...
    async getEmploySimple() {
      this.Data = await getManagerListApi() // gaolyQQ需要提供该方法
    },
    // 弹层显示
    dialogFormV() {
